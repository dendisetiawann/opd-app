                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <x-text-input id="domain" class="block mt-1 w-full" type="text" name="domain" :value="old('domain', $webApp->domain)" />
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <x-text-input id="bahasa_backend" class="block mt-1 w-full" type="text" name="bahasa_backend" :value="old('bahasa_backend', $webApp->bahasa_backend)" />
                            </div>
                            <div>
                                <x-input-label for="versi_backend" :value="__('Versi Backend')" />
                                <x-text-input id="versi_backend" class="block mt-1 w-full" type="text" name="versi_backend" :value="old('versi_backend', $webApp->versi_backend)" />
                            </div>
                            <div>
                                ...
                                <select id="arsitektur_sistem" name="arsitektur_sistem" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                    ...
                                    <option value="monolith" {{ old('arsitektur_sistem', $webApp->arsitektur_sistem) == 'monolith' ? 'selected' : '' }}>Monolith</option>
                                    <option value="be-fe" {{ old('arsitektur_sistem', $webApp->arsitektur_sistem) == 'be-fe' ? 'selected' : '' }}>Backend-Frontend</option>
                                </select>
                            </div>
                            <div>
                                <x-input-label for="bahasa_frontend" :value="__('Bahasa Frontend')" />
                                <x-text-input id="bahasa_frontend" class="block mt-1 w-full" type="text" name="bahasa_frontend" :value="old('bahasa_frontend', $webApp->bahasa_frontend)" />
                            </div>
                            <div>
                                <x-input-label for="versi_frontend" :value="__('Versi Frontend')" />
                                <x-text-input id="versi_frontend" class="block mt-1 w-full" type="text" name="versi_frontend" :value="old('versi_frontend', $webApp->versi_frontend)" />
                            </div>
                            <div>
                                <x-input-label for="framework" :value="__('Framework')" />
                                <x-text-input id="framework" class="block mt-1 w-full" type="text" name="framework" :value="old('framework', $webApp->framework)" />
                            </div>
                            <div>
                                <x-input-label for="versi_framework" :value="__('Versi Framework')" />
                                <x-text-input id="versi_framework" class="block mt-1 w-full" type="text" name="versi_framework" :value="old('versi_framework', $webApp->versi_framework)" />
                            </div>
                            <div class="md:col-span-2">
                                <x-input-label for="daftar_library_package" :value="__('Daftar Library / Package')" />
                                <textarea id="daftar_library_package" name="daftar_library_package" rows="2" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('daftar_library_package', $webApp->daftar_library_package) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
