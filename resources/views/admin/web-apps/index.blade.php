    ...
    <div class="relative bg-white rounded-2xl p-6 mb-6 overflow-hidden shadow-sm border border-gray-100">
        ...
        <div class="relative flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-xl font-bold text-slate-800">Data Aplikasi Semua OPD</h2>
                    <p class="text-sm text-slate-500">Daftar lengkap data aplikasi yang dikelola oleh seluruh OPD.</p>
                </div>
            </div>
            <div class="flex items-center gap-3">
                <div class="flex items-center gap-2 bg-gradient-to-r from-blue-50 to-cyan-50 px-4 py-2 rounded-xl border border-blue-100">
                    ...
                    <span class="text-sm text-slate-600">Total <span class="font-bold text-blue-600">{{ $webApps->total() }}</span> aplikasi terdaftar dari seluruh OPD</span>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="p-6">
            <form id="webAppFilterForm" method="GET" action="{{ route('admin.web-apps.index') }}" class="flex flex-wrap gap-4 items-end">
                <div class="flex-1 min-w-[180px]">
                    ...
                    <div class="relative">
                        <select name="opd_id" id="opd_id" onchange="document.getElementById('webAppFilterForm').submit()" class="w-full border-gray-200 focus:border-blue-500 focus:ring-blue-500 rounded-xl shadow-sm text-sm pl-10 py-2.5 bg-gray-50 hover:bg-white transition-colors">
                ...
                <div class="w-full sm:w-auto">
                    ...
                    <div class="relative">
                        <select name="sort" id="sort" onchange="this.form.submit()"
                            class="w-full pl-10 pr-10 py-2.5 border-gray-200 focus:border-purple-500 focus:ring-purple-500 rounded-xl shadow-sm text-sm appearance-none bg-gray-50 hover:bg-white transition-colors cursor-pointer">
                ...
                <div class="flex gap-2">
                    ...
                    <a id="adminWebAppResetBtn" href="{{ route('admin.web-apps.index') }}" class="inline-flex items-center px-4 py-2.5 bg-gray-100 border border-gray-200 rounded-xl font-semibold text-sm text-gray-600 hover:bg-gray-200 hover:text-gray-700 transition-all {{ (request('search') || request('opd_id')) ? '' : 'hidden' }}">
                            ...
                            @foreach($webApps as $index => $app)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-4 py-4 text-center">
                                        <span class="inline-flex items-center justify-center w-7 h-7 bg-slate-100 text-slate-600 text-xs font-bold rounded-full">{{ $webApps->firstItem() + $index }}</span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div>
                                            <div class="text-sm font-bold text-gray-900">{{ $app->nama_web_app }}</div>
                                            @if($app->alamat_tautan)
                                                <a href="{{ str_starts_with($app->alamat_tautan, 'http') ? $app->alamat_tautan : 'http://' . $app->alamat_tautan }}" target="_blank" class="inline-flex items-center gap-1 text-xs text-blue-600 hover:text-blue-700 font-medium bg-blue-50 px-2 py-0.5 rounded-md hover:bg-blue-100 transition-colors mt-1">
